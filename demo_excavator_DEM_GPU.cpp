...
#include "irrlicht_interface/CHbodySceneNode_GPUDEM.h"
#include "irrlicht_interface/CHbodySceneNodeTools_GPUDEM.h"
#include "irrlicht_interface/CHirrAppInterface_GPUDEM.h"
...
#include "unit_gpu/CHcontactContainerGPUDEM.h"
//#include "collision/CHcModelBulletDEM.h"
#include "lcp/CHlcpSolverDEM.h"

#include "unit_GPU/CHbodyGPUDEM.h"
//#include "physics/CHcontactContainerDEM.h"
#include "unit_GPU/CHcCollisionModelGPUDEM.h"
#include "unit_GPU/CHcCollisionSystemGPUDEM.h"

#define springCoef 1000 /*39240.0*/
#define dampingCoef 50000 /*42000*/
//#define springCoef 392400.0
//#define dampingCoef 42000
                ...
                for (int k = 0; k < depth; k++) {
                        for (int i = 0; i < width; i++) {
                                if (bi < numParticles) {
                                        ...
                                        ChSharedPtr<ChBodyDEM> bodyPtr = (ChSharedPtr<ChBodyDEM>)mrigidBody->GetBody();
                                        bodyPtr->SetSpringCoefficient(springCoef);
                                        bodyPtr->SetDampingCoefficient(dampingCoef);
...
int main(int argc, char* argv[])
{
        ...
        ChSystem mphysicalSystem(60000, 500);                   //gpu unnecessary
        ...
        mphysicalSystem.Set_G_acc (ChVector<>(0.0, -9800.0, 0.0));
                ...
                //ChSharedPtr<ChContactContainerDEM> my_nodes_container(new ChContactContainerDEM);
                //mphysicalSystem.ChangeContactContainer(my_nodes_container.get_ptr());

                ChCollisionSystemGPUDEM my_c_sys(0);
                mphysicalSystem.ChangeCollisionSystem(&my_c_sys);
                ChSharedPtr<ChContactContainerGPUDEM> my_nodes_container(new ChContactContainerGPUDEM);
                mphysicalSystem.ChangeContactContainer(my_nodes_container.get_ptr());
        ...
        //ChBodySceneNode* scoop = (ChBodySceneNode*)addChBodySceneNode_easyConcaveMesh2(
        //                                                                      application.GetSystem(),
        //                                                                      application.GetSceneManager(),
        //                                                                      "../data/bucket_mm.obj",
        //                                                                      "../data/bucketFinalSpheres.txt",
        //                                                                      0.4,
        //                                                                      ChVector<>(-447,45,29),
        //                                                                      QUNIT);

        //ChSharedPtr<ChBodyDEM> scoop_Ptr = (ChSharedPtr<ChBodyDEM>)scoop->GetBody();
        //scoop_Ptr->GetCollisionModel()->SetEnvelope(0);
        //scoop_Ptr->SetSpringCoefficient(springCoef);
        //scoop_Ptr->SetDampingCoefficient(dampingCoef);
        //scoop_Ptr->SetMass(0.3 * massScale);
        //scoop_Ptr->SetInertiaXX(ChVector<>(290000,360000,260000) * massScale);
        ////scoop_Ptr->SetBodyFixed(true);
        //scoop_Ptr->SetCollide(true);
        //scoop_Ptr->GetCollisionModel()->SetFamily(3);
        //scoop_Ptr->GetCollisionModel()->SetFamilyMaskNoCollisionWithFamily(3);
        //scoop->setMaterialTexture(0,excavatorMap);


        //ChBodySceneNode* forearm = (ChBodySceneNode*)addChBodySceneNode_easyConcaveMeshDum(
        //                                                                      application.GetSystem(),
        //                                                                      application.GetSceneManager(),
        //                                                                      "../data/forearm_mm.obj",
        //                                                                      1.7,
        ...
        //ChBodySceneNode* upperarm = (ChBodySceneNode*)addChBodySceneNode_easyConcaveMeshDum(
        //                                                                      application.GetSystem(),
        //                                                                      application.GetSceneManager(),
        ...
        //ChBodySceneNode* body = (ChBodySceneNode*)addChBodySceneNode_easyConcaveMeshDum(
        //                                                                      application.GetSystem(),
        //                                                                      application.GetSceneManager(),
        //                                                                      "../data/body_mm.obj",
        //                                                                      2*17,
        ...
        //body->GetBody()->SetInertiaXX(2*ChVector<>(51000000,100000000,71000000) * massScale);
        ...
        //ChBodySceneNode* drive = (ChBodySceneNode*)addChBodySceneNode_easyConcaveMeshDum(
        //                                                                      application.GetSystem(),
        //                                                                      application.GetSceneManager(),
        //                                                                      "../data/drive_mm.obj",
        //                                                                      0.2*5,
        ...
        //ChBodySceneNode* tracks = (ChBodySceneNode*)addChBodySceneNode_easyConcaveMeshDum(
        //                                                                      application.GetSystem(),
        //                                                                      application.GetSceneManager(),
        //                                                                      "../data/tracks_mm.obj",
        //                                                                      2*5,
        ...
        //tracks->GetBody()->SetCollide(false);
        ...
        ChSharedPtr<ChBodyGPUDEM> mrigidBody_Ptr;

        ChBodySceneNode* container = (ChBodySceneNode*)addChBodySceneNode_easyConcaveMesh2(
                                                                                application.GetSystem(),
                                                                                application.GetSceneManager(),
                                                                                "../data/containerShow.obj",
                                                                                "../data/containerFinalSpheres.txt",
                                                                                0.4,
                                                                                ChVector<>(-500,-96, 30),//(300,5,120),
                                                                                QUNIT);

        ChSharedPtr<ChBodyDEM> container_Ptr = (ChSharedPtr<ChBodyDEM>)container->GetBody();
        container_Ptr->GetCollisionModel()->SetEnvelope(0);
        container_Ptr->SetSpringCoefficient(springCoef);
        container_Ptr->SetDampingCoefficient(dampingCoef);
        container_Ptr->SetMass(10 * massScale);
        container_Ptr->SetInertiaXX(ChVector<>(1000000,1000000,1000000) * massScale);
        container_Ptr->SetBodyFixed(true);
        container_Ptr->SetCollide(true);
        container_Ptr->GetCollisionModel()->SetFamily(3);
        container_Ptr->GetCollisionModel()->SetFamilyMaskNoCollisionWithFamily(3);
        container->setMaterialTexture(0,floorMap);
/////////
...
///////
   ...
   int savestep = 10 * periodExtension;
       ...
       fstream outFileTime;
       outFileTime.open("simTimeDEM_GPU.txt", ios::out);

       while(application.GetDevice()->run() && cumulStep < endWrite * 3) {
               step++;
               cumulStep++;
               //printf("** step %d\n", step);

               if ((cumulStep) % (200 * int(periodExtension)) == 1 && cumulStep > 0 && cumulStep < 500 * int(periodExtension)) {
                       ...
                       create_some_falling_items(&application, mphysicalSystem, application.GetSceneManager(), application.GetVideoDriver());
                                                                                                                               ...
                                                                                                                               //if ((cumulStep - 0) % (200 * int(periodExtension)) == 1 && cumulStep > 0 && cumulStep < startWrite) {
               ...
               //ChFunction_Const * torqFunc3 = (ChFunction_Const*)my_motor3->Get_tor_funct();
                ...
                //prevent increase in collision domain
                if (cumulStep % (10 * int(periodExtension)) == 0 ) {
                        std::vector<ChBody*>::iterator ibody = mphysicalSystem.Get_bodylist()->begin();
                        while (ibody != mphysicalSystem.Get_bodylist()->end()) {
                                ChVector<> bodyPos = (*ibody)->GetPos();
                                ChVector<> bodyVel = (*ibody)->GetPos_dt();
                                if ((*ibody)->GetCollisionModel()->GetFamily() < 0) {
                                        //if (bodyPos.y < -110) { bodyPos.y = -110;     (*ibody)->SetPos(bodyPos); (*ibody)->SetBodyFixed(true); }
                                        //if (bodyPos.y > 100) { bodyPos.y = -130;      (*ibody)->SetBodyFixed(true); }
                                        //if (bodyPos.x < -850) { bodyPos.x = -850; /*bodyPos.y = -110;*/       (*ibody)->SetBodyFixed(true); }
                                        //if (bodyPos.x > -150) { bodyPos.x = -150; /*bodyPos.y = -110;*/       (*ibody)->SetBodyFixed(true); }
                                        //if (bodyPos.z < -150) { bodyPos.z = -150;     /*bodyPos.y = -105;*/   (*ibody)->SetBodyFixed(true); }
                                        //if (bodyPos.z > 180) { bodyPos.z = 180;       /*bodyPos.y = -115;*/   (*ibody)->SetBodyFixed(true); }
                                        //(*ibody)->SetPos(bodyPos);

                                        if ((bodyPos.y < -110) || (bodyPos.y > 100)) { bodyVel.y = 0; }
                                        if ((bodyPos.x < -850) || (bodyPos.x > -150)) { bodyVel.x = 0; }
                                        if ((bodyPos.z < -150) || (bodyPos.z > 180)) { bodyVel.z = 0; }
                                        (*ibody)->SetPos_dt(bodyVel);
                                }
                                ibody++;
                        }
                }
